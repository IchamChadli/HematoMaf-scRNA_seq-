...
{ currentFeature <- as.character(currentFeature)
  ...
  featureValues = rowMeans(as.matrix(FetchData(object = seuratObject, vars = currentFeature, slot = slot)));
  ...
  ggFigure = ggplot( data.frame( Counts = featureValues, Cluster = Idents( seuratObject)), 
               ...
               geom_jitter( width = 0.2, height = 0, size = 0.5, colour = "#44444444") +
